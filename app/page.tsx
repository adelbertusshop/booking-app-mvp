...
function BookingFormContent() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!salonSlug) {
      setMessage({
        type: 'error',
        text: 'Brak identyfikatora salonu w adresie URL. Dodaj ?salon=nazwa-salonu do adresu strony.'
      });
      return;
    }
...
export default function Home() {
  return (
    <div className="min-h-screen bg-black text-amber-100 flex items-center justify-center p-4">
      <Suspense fallback={<div className="text-amber-400">Ładowanie formularza...</div>}>
        <BookingFormContent />
      </Suspense>
    </div>
  );
