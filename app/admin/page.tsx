...
export default function AdminPage() {
  ...
  const loadSalonData = async (currentUser: any) => {
    ...
    let { data: salon } = await supabase
      .from('salons').select('*').eq('user_id', currentUser.id).single();
  ...
  const fetchAppointments = async (sid: string) => {
    const { data } = await supabase
      .from('appointments').select('*').eq('salon_id', sid)
  ...
  const fetchServices = async (sid: string) => {
    const { data } = await supabase
      .from('services').select('*').eq('salon_id', sid).order('name', { ascending: true });
    if (data) setServices(data as Service[]);
  };

  const handleDeleteAppointment = async (appt: Appointment) => {
    if (!confirm(`Usunąć rezerwację klienta ${appt.client_name}?`)) return;
    setDeletingId(appt.id);

    // Ustaw status cancelled
    await supabase.from('appointments').update({ status: 'cancelled' }).eq('id', appt.id);

    // Email o odwołaniu do klienta
  ...
  const handleAddService = async (e: React.FormEvent) => {
    ...
    if (error) { alert('Błąd: ' + error.message); }
      ...
      setNewServiceName(''); setNewServiceDuration('60'); setNewServicePrice('');
    }
  ...
  const handleAuth = async (e: React.FormEvent) => {
    ...
    if (isRegistering) {
      ...
      if (data?.user) {
        // Utwórz salon od razu z nazwą
        await supabase.from('salons').insert([{
          user_id: data.user.id,
          admin_email: email.trim(),
          salon_name: salonNameReg.trim(),
          whatsapp_template: 'Cześć {NAME}! Przypominamy o wizycie: {SERVICE} w dniu {DATE} o godz. {TIME}. Do zobaczenia!',
        }]);
        alert('Konto zarejestrowane! Sprawdź email i zaloguj się.');
  ...
  const handleSaveSettings = async (e: React.FormEvent) => {
    ...
    if (salonId) {
      const { error } = await supabase.from('salons')
        .update({ salon_name: salonName, admin_email: adminEmail, whatsapp_template: whatsappTemplate })
        .eq('id', salonId);
  ...
  if (loadingAuth) {
    return (
      <main className="min-h-screen bg-black flex items-center justify-center">
        <p className="text-amber-400 font-bold animate-pulse">Ładowanie...</p>
      </main>
    );
  }
        ...
        <form onSubmit={handleAuth} className="bg-zinc-950 border border-amber-500/30 p-8 rounded-2xl max-w-sm w-full space-y-4 shadow-2xl">
          ...
          {isRegistering && (
            <div>
              ...
              <input
                type="text"
                placeholder="np. Salon Klaudia"
                value={salonNameReg}
                onChange={(e) => setSalonNameReg(e.target.value)}
          ...
          <div>
            ...
            <div className="relative">
              <input
                ...
                className="w-full bg-zinc-900 border border-amber-500/30 rounded-lg p-3 pr-12 text-amber-100 text-sm focus:outline-none focus:border-amber-400"
                required
              />
              <button
                type="button"
          ...
          <button
            type="button"
            onClick={() => { setError(''); setSalonNameReg(''); setIsRegistering(!isRegistering); }}
    ...
    <main className="min-h-screen bg-black text-amber-100 p-4 md:p-8">
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="flex space-x-6 border-b border-zinc-800 pb-2">
          {(['appointments', 'services', 'settings'] as const).map((tab) => (
            <button key={tab} onClick={() => setActiveTab(tab)}
              className={`text-sm font-semibold pb-1 transition-all ${activeTab === tab ? 'text-amber-400 border-b-2 border-amber-400' : 'text-zinc-500 hover:text-amber-200'}`}>
        ...
        {activeTab === 'appointments' && (
          <div>
            {loadingData ? (
              <p className="text-amber-200 text-sm animate-pulse">Wczytywanie...</p>
          ...
          <div className="space-y-6">
            <div className="bg-zinc-950 border border-amber-500/30 rounded-2xl p-6 shadow-2xl">
              ...
              <form onSubmit={handleAddService} className="grid grid-cols-1 md:grid-cols-4 gap-3">
                <input type="text" placeholder="Nazwa usługi" value={newServiceName}
                ...
                <input type="number" placeholder="Czas (min)" value={newServiceDuration}
                  onChange={(e) => setNewServiceDuration(e.target.value)} min="15" step="15"
                ...
                <input type="number" placeholder="Cena PLN" value={newServicePrice}
                  onChange={(e) => setNewServicePrice(e.target.value)} min="0" step="0.01"
            ...
            <div className="bg-zinc-950 border border-amber-500/30 rounded-2xl p-6 shadow-2xl">
              ...
              {services.length === 0 ? (
                <p className="text-zinc-500 text-sm text-center py-6">Brak usług. Dodaj pierwszą powyżej.</p>
              ) : (
          ...
          <form onSubmit={handleSaveSettings} className="bg-zinc-950 border border-amber-500/30 rounded-2xl p-6 max-w-xl space-y-4 shadow-2xl">
            ...
            <div className="bg-zinc-900 border border-amber-500/20 rounded-lg p-3">
              ...
              <p className="text-xs text-zinc-400 break-all">
                {typeof window !== 'undefined' ? window.location.origin : 'https://twoja-domena.vercel.app'}/?salon={salonId || 'twoje-id'}
              </p>
            </div>
