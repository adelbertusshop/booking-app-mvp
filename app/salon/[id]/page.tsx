...
interface Salon { id: string; salon_name: string; }
...
export default function SalonPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    async function init() {
      const { data: salonData, error } = await supabase.from('salons').select('id, salon_name').eq('id', salonId).single();
      if (error || !salonData) { setNotFound(true); setLoadingInit(false); return; }
      setSalon(salonData);
      const { data: svcs } = await supabase.from('services').select('id, name, duration_minutes, price').eq('salon_id', salonId).order('name');
      const list = svcs || [];
      setServices(list);
  ...
  useEffect(() => {
    if (!date || !salonId) { setAvailableSlots([]); setSelectedTime(''); setIsDayOff(false); return; }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const res = await fetch('/api/appointments', {
      ...
      body: JSON.stringify({ date, time: selectedTime, clientName, email, phone, salonId, serviceId: selectedServiceId || null }),
    });
  ...
  if (loadingInit) return <div className="min-h-screen bg-black flex items-center justify-center"><p className="text-amber-400 animate-pulse font-bold">Ładowanie...</p></div>;
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div>
              ...
              {!date ? <p className="text-xs text-zinc-500">Najpierw wybierz dzień.</p>
                ...
                : isDayOff ? <p className="text-xs text-red-400">Ten dzień jest dniem wolnym. Wybierz inny termin.</p>
